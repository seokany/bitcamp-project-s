...
CREATE TABLE `MENTO` (
  ...
  `carr`  varchar(50) NULL     COMMENT '경력' -- 경력
)
COMMENT '멘토';
...
CREATE TABLE `JOB` (
  ...
  `jbimg` VARBINARY(1000) NOT NULL COMMENT '직업썸네일' -- 직업썸네일
)
COMMENT '직업';
...
CREATE TABLE `PERSON` (
  ...
  `psimg1` VARBINARY(1000) NOT NULL COMMENT '인물썸네일1', -- 인물썸네일1
  `psimg2` VARBINARY(1000) NOT NULL COMMENT '인물썸네일2', -- 인물썸네일2
...
CREATE TABLE `QNA` (
  `cono` INTEGER NOT NULL COMMENT '설계도일련번호', -- 설계도일련번호
  `sno`  INTEGER NOT NULL COMMENT '학생일련번호', -- 학생일련번호
  `chat` BLOB    NOT NULL COMMENT '내용' -- 내용
)
COMMENT '질의응답';
